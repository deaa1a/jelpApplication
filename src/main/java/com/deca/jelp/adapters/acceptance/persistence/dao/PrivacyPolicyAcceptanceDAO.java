...
@Repository
public interface PrivacyPolicyAcceptanceDAO extends JpaRepository<PrivacyPolicyRecord, String> {

    @Query(
            "UPDATE PrivacyPolicyRecord SET updateDate = : updateDate, accepted = :accepted WHERE customerId = :customerId")
    PrivacyPolicyRecord findBy(
    ...
    void updatePrivacyPolicy(Boolean accepted, ZonedDateTime updateDate);
}
